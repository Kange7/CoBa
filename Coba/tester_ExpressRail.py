...
class ModelTester:
    def __init__(self, model, dataset, restore_snap=None):
        ...
        self.Log_file = open('log_test_' + str(dataset.val_file_name) + '.txt', 'a')
            ...
            try:
                ...
                if last_min + 1 < new_min:
                    ...
                    for i_test in range(num_val):
                        ...
                        # print(dir(dataset))  所有存在的模块输出
                        # print(dataset.input_xyz)
                    ...
                    if int(np.ceil(new_min)) % 1 == 0:
                        ...
                        for i_test in range(num_val):
                            ...
                            preds = dataset.label_values[np.argmax(proj_probs_list[i_test], axis=1)].astype(np.uint8) # 1 (10283800,)

                            # Confusion
                            labels = dataset.val_labels[i_test]  # 2 (10283800,)
