...
if __name__ == '__main__':
    ...
    # H3D
    print('Dataset: Pairs3D')
    logs = [

        'Log_2025-05-24_03-37-34',
    
    ]
    
    for log in logs:
        print(log)
        ply_file = '/home/joe/program/U-Next/test/' + log + '/val_preds/Lille2.ply'
        # ply_file = '/home/joe/program/program_2/test/Log_2024-11-20_06-06-21(K-ISPRS)/predictions/Vaihingen3D_EVAL_WITH_REF.ply'
        data = read_ply(ply_file)
        gp = np.vstack((data['label'], data['pred'])).T
    ...
    '''
    # ExpressRail
    print('Dataset: ExpressRail')
    logs = [

        'Log_2025-04-23_00-03-04',
    
    ]
    cloud_names = ['L02-label-1',
                   'L02-label-3',
                   'L02-label-5',
                   'L04-label-4',
                   'L06-label-3',
    ...
    for log in logs:
        ...
        for name in cloud_names:
            ply_file = '/home/joe/program/U-Next/test/' + log + '/val_preds/'+name+'.ply'
            data = read_ply(ply_file)
            gp.extend(np.vstack((data['label'], data['pred'])).T)
